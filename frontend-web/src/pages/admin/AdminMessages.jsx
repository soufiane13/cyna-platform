        ...
        <div className="p-8 min-h-screen bg-[#0B0E14] text-white animate-fade-in relative">
            ...
            <div className="bg-[#1C2128] border border-[#2D333B] p-4 rounded-t-2xl flex items-center justify-between gap-4">
                <div className="relative w-full sm:w-[300px]">
                    ...
                    <input type="text" id="search-messages" name="search" placeholder="Rechercher (Email, Contenu)..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full bg-[#0B0E14] border border-[#2D333B] rounded-lg h-10 pl-10 pr-4 text-sm text-white focus:outline-none focus:border-cyna-cyan" />
                </div>
                ...
                <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 animate-fade-in">
                    <div className="bg-[#1C2128] border border-[#2D333B] w-full max-w-2xl rounded-2xl overflow-hidden shadow-2xl relative">
                        ...
                        <div className="p-6 space-y-4">
                            <div><label htmlFor="reply-to" className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Destinataire</label><input type="text" id="reply-to" name="to" value={emailData.to} disabled className="w-full bg-[#0B0E14] border border-[#2D333B] rounded-lg p-3 text-sm text-gray-500 cursor-not-allowed" /></div>
                            <div><label htmlFor="reply-subject" className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Sujet</label><input type="text" id="reply-subject" name="subject" value={emailData.subject} onChange={e => setEmailData({...emailData, subject: e.target.value})} className="w-full bg-[#0B0E14] border border-[#2D333B] rounded-lg p-3 text-sm text-white focus:outline-none focus:border-cyna-cyan" /></div>
                            <div><label htmlFor="reply-message" className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Votre Réponse</label><textarea id="reply-message" name="message" value={emailData.message} onChange={e => setEmailData({...emailData, message: e.target.value})} className="w-full h-48 bg-[#0B0E14] border border-[#2D333B] rounded-lg p-3 text-sm text-white focus:outline-none focus:border-cyna-cyan resize-none" placeholder="Tapez votre réponse ici..." /></div>
